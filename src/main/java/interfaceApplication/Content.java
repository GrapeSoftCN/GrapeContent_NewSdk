...
public class Content {
	...
	public Content() {
		...
		if (userInfo != null && userInfo.size() != 0) {
			...
			userName = userInfo.getString("name"); // 当前用户姓名
			userId = userInfo.getString("id"); // 当前用户用户名
	...
	public String read_by_id(String _id) {
		...
		JSONObject find = content.eq(pkString, _id).find();
		if (find != null) {
	 ...
	 *
	 */
	public String CheckLink(final int idx, final int pageSize) {

		final distributedLocker countLocker = distributedLocker.newLocker("BadlinkAndDiedlink_" + currentWeb);
		final appIns currentAppInfo = appsProxy.getCurrentAppInfo();
		String rString = null;
		long neartime1 = 0;
		CacheHelper ch = new CacheHelper();

		rString = ch.get("BadlinkAndDiedlink_" + currentWeb);
		JSONObject jsobj1 = JSONObject.toJSON(rString);
		if (jsobj1 != null) {
			String neartime = jsobj1.getString("0");
			neartime1 = Long.parseLong(neartime);
		}
		if (countLocker != null) {
			boolean flag = countLocker.lock();
			if (flag) {// 如果锁定成功

				if (rString == null || neartime1 == 0 || TimeHelper.nowMillis() - neartime1 >= 86400000) {

					new Thread(new Runnable() {
						@Override
						public void run() {
							try {
								JSONArray rString1 = CheckLink1();
								long nowMillis = TimeHelper.nowMillis();
								JSONObject puts = new JSONObject().puts("0", nowMillis).puts("1", rString1);
								ch.setNX("BadlinkAndDiedlink_" + currentWeb, puts.toJSONString());
		...
		if (!StringHelper.InvaildString(rString)) {
			...
			if (jsobj1 != null) {
				String object = jsobj1.getString("1");
				...
				rString = rMsg.netPAGE(idx, pageSize, size, jsonarray_page);
			}
		}
		return rString;
	}
	...
	private JSONArray CheckLink1() {
		JSONArray rArray = content.eq("wbid", currentWeb).eq("isdelete", 0).field("url,ogid,mainName,time")
				.scan((array) -> {
					JSONArray _rArray = new JSONArray();
					if (array.size() > 0 && array != null) {
						for (Object object : array) {
							Print_nlogger.Print_SYSO(a++);
							...
							try {
								...
								if ("host parameter is null".equals(e.getMessage())) {
							...
							GrapeTreeDBModel group = contentGroup.getGroup();
							JSONObject obj = group.eq(pkString, ogid).field("name").find();
		...
		// JSONArray jsonarray_page = JSONArrayUtils.jsonarray_page(rArray, idx,
		// pageSize);
		// return rMsg.netPAGE(idx, pageSize, size, jsonarray_page);
		return rArray;
	...
	/**
	 * 把一个栏目下的所有文章移动到另外一个栏目(全部)
	 * 
	 * @param wbid
	 * @return
	 */
	public String move_articles_to_another_ogid(String wbid, String sourcogid, String targetogid) {
		long updateAll = content.eq("wbid", wbid).eq("ogid", sourcogid).data(new JSONObject("ogid", targetogid))
				.updateAll();
	...
	public String move_articles_to_another_ogid_SELECT(String ids, String targetogid) {
		...
		content.or();
		for (String id : split) {
			content.eq(pkString, id);
		}
		long updateAll = content.eq("isdelete", 0).data(new JSONObject("ogid", targetogid)).updateAll();
	...
	private String AddAll(JSONObject object) {
		...
		tip = (JSONHelper.string2json(info) != null && !info.contains("errorcode"))
				? tip = content.data(info).autoComplete().insertOnce()
				: null;
	...
	public String ContentIsExist(String ogid, String mainName, String time, int type) {
		...
		return ContentIsExist(ogid, mainName, time, type, contentInfo);
	}
	...
	public String ContentIsExist(String ogid, String mainName, String time, int type, String contentInfo) {
		...
		if (StringHelper.InvaildString(ogid)) {
			...
			if (type == 0) {
				content.ne("content", contentInfo);
			} else {
				content.ne("contenturl", contentInfo);
			}
	...
	// * @return 返回0:表示文章不存在；返回1:表示文章已存在；
	// */
	// public String ContentIsExist(String ogid, String mainName,String time, int
	// type, String contentInfo) {
	// JSONObject object = null;
	// contentInfo = codec.DecodeHtmlTag(contentInfo);
	// contentInfo = codec.decodebase64(contentInfo);
	//
	// if (StringHelper.InvaildString(ogid)) {
	// content.eq("ogid", ogid).eq("mainName",
	// codec.DecodeFastJSON(mainName)).eq("isdelete", 0);
	// if (type == 0) {
	// content.eq("content", contentInfo);
	// } else {
	// content.eq("contenturl", contentInfo);
	...
	public String PublishArticle(String ArticleInfo) {
		...
		String insert = insert(object);
		// 根据返回值添加日志
		JSONObject json = JSONObject.toJSON(insert);
		if (0 == json.getInt("errorcode")) {

			LogsUtils.addLogs(userId, userName, "在[" + ogname + "]栏目下发布了[" + object.getString("mainName") + "]新闻",
					currentWeb, "PublishArticle");
		}
		return insert;
	}
	...
	public String EditArticle(String oid, String contents) {
		...
		if (infos != null && infos.size() > 0) {
			...
			// 验证当前文章内容是否修改过
			JSONObject obj = content.eq("_id", oid).eq("state", 1).find();
			if (obj != null && obj.size() > 0) {
				infos.put("state", 0);
			}
			// if (infos.containsKey("content")) {
			// contentInfos = infos.getString("content");
			// JSONObject obj = content.eq("_id", oid).eq("content", contentInfos).find();
			// System.out.println(obj == null || obj.size() <= 0);
			// if (obj == null || obj.size() <= 0) {
			// infos.put("state", 0);
			// }
			// }
	...
	public Object ExportContent_ckd(String wbid, String ContentInfo, String fileName) {
		try {
			...
			ContentInfo = SearchExportInfo_ckd(ContentInfo, wbid);
			if (StringHelper.InvaildString(ContentInfo)) {
				return excelHelper.out(ContentInfo);
			}
		} catch (Exception e) {
	...
	private String SearchExportInfo_ckd(String condString, String wbid) {
		...
		if (StringHelper.InvaildString(condString)) {
			...
			array = content.eq("wbid", wbid).where(condArray).field("wbid").select();
	...
	private String SearchExportInfo(String condString) {
		...
		if (StringHelper.InvaildString(condString)) {
			JSONArray condArray = JSONArray.toJSONArray(condString);
			array = content.eq("wbid", currentWeb).where(condArray).field("mainName,state,souce,author,time,ogid")
					.select();
			if (array != null && array.size() > 0) {
				// 获取栏目id
				for (Object object : array) {
	...
	public String FindNewArc(String wbid, int idx, int pageSize) {
		long total = 0;
		JSONArray array = null;
		content.eq("wbid", model.getRWbid(wbid)).eq("state", 2).eq("isdelete", 0).eq("isvisble", 0).desc("isTop")
				.desc("time").desc(pkString);
		...
		return rMsg.netPAGE(idx, pageSize, total,
				(array != null && array.size() > 0) ? model.join(array) : new JSONArray());
	}
	...
	public String findAllArticles(String ids) {
		String tempID = "";
		JSONObject rJsonObject = new JSONObject(), tempJson;
		String[] value = ids.split(",");
		for (String string : value) {
			if (StringHelper.InvaildString(string)) {
				if (ObjectId.isValid(string)) {
					content.or().eq(pkString, string);
				}
			}
		}
		JSONArray array = content.and().eq("isdelete", 0).eq("isvisble", 0).field("mainName,content,author,souce,time")
				.select();
		for (Object object : array) {
			tempJson = (JSONObject) object;
			tempID = tempJson.getMongoID(pkString);
			rJsonObject.put(tempID, model.ContentDencode(tempJson));
		}
	...
	public String ShowPicByGroupId(String wbid, String ogid) {
		...
		try {
			...
			if (condArray != null && condArray.size() > 0 && StringHelper.InvaildString(wbid)) {
				array = content.or().where(condArray).and().eq("wbid", wbid).eq("slevel", 0).eq("isdelete", 0)
						.eq("isvisble", 0).field("_id,mainName,ogid,time,image,contenturl").desc("isTop").desc("time")
						.desc("sort").desc("_id").limit(100).select();
	...
	public String Page(String wbid, int idx, int pageSize) {
		...
		JSONArray array = content.eq("isdelete", 0).eq("isvisble", 0)
				.field("_id,mainName,time,wbid,ogid,image,clickcount,souce,contenturl").desc("isTop").desc("time")
				.desc("sort").desc("_id").page(idx, pageSize);
		array = model.setTemplate(array);
	...
	public String PageBy(String wbid, int idx, int pageSize, String condString) {
		...
		if (content.getCondCount() > 0) {
			...
			array = content.desc("isTop").desc("time").desc("sort").desc("_id").field(
					"_id,mainName,time,wbid,ogid,image,clickcount,souce,contenturl,isTop,subName,content,desp,author,TemplateContent,Templatelist")
					.page(idx, pageSize);
	...
	public String jian_cha_chong_fu(int time_interval, String ogids) {
		...
		for (String ogid : ogidArr) {
			JSONObject jian_cha_chong_fu1 = jian_cha_chong_fu1(time_interval, ogid, null, null);
	...
	public String jian_cha_chong_fu(int time_interval, String ogids, String starttime, String endtime) {
		...
		String[] ogidArr = ogids.split(",");
		for (String ogid : ogidArr) {
			JSONObject jian_cha_chong_fu1 = jian_cha_chong_fu1(time_interval, ogid, starttime, endtime);
			if (jian_cha_chong_fu1 != null && jian_cha_chong_fu1.size() > 0) {
				jsonArray.adds(jian_cha_chong_fu1);
			}
		}
	...
	public JSONObject jian_cha_chong_fu1(int time_interval, String ogid1, String starttime, String endtime) {
		...
		content.eq("ogid", ogid1).and().where(build2).field("mainName,ogid,url,time");

		final HashMap<String, Long> hashMap = new HashMap<String, Long>();
		JSONObject jsonObject = new JSONObject();
		final HashMap<String, HashSet<Long>> timeMap = new HashMap<String, HashSet<Long>>();
		JSONArray rArray = content.scan((array) -> {
			JSONArray _rArray = new JSONArray();
			for (Object object : array) {
				JSONObject ob = (JSONObject) object;
				Long time = ob.getLong("time");
				String ogid = ob.getString("ogid");
				String mainName = ob.getString("mainName");

				Long put = hashMap.put(mainName, time);

				if (put != null) {
					HashSet<Long> hashSet = timeMap.get(mainName);
					if (hashSet != null && hashSet.size() > 0) {
						for (Long timeexist : hashSet) {
							Long day = Math.abs(time - timeexist) / 86400000;
							if (day <= time_interval || time_interval == -1) {
								...
								String ogname = contentGroup.getOgname_By_ogid(ogid);
			...
			if (flag) {// 如果锁定成功
				...
				if (rString == null || rString.equals("")) {
					...
					JSONObject webInfo = JSONObject
							.toJSON((String) appsProxy.proxyCall("/GrapeWebInfo/WebInfo/getWebInfo/s:" + rootID));
	...
	public void test1() {
		...
		cacheHelper.setget("ckdtest1", "陈凯迪", 30);
	}

	// public String totalArticle(String rootID) {
	// distributedLocker countLocker = distributedLocker.newLocker("totalArticle_" +
	// rootID);
	// String rString = "";
	// if (countLocker != null) {
	// boolean flag = countLocker.lock();
	...
	// JSONObject json = new JSONObject();
	// JSONObject webInfo = JSONObject
	// .toJSON((String) appsProxy.proxyCall("/GrapeWebInfo/WebInfo/getWebInfo/s:" +
	// rootID));
	// json = new WsCount().getAllCount(json, rootID, webInfo.getString(rootID),
	// "");
	// rString = json.toJSONString();
	...
	public String total(String rootID, String starTime, String endTime) {
		...
		if (countLocker.lock()) {
			...
			if (rString == null || rString.equals("")) {
				JSONObject json = new JSONObject();
				JSONObject webInfo = JSONObject
						.toJSON(appsProxy.proxyCall("/GrapeWebInfo/WebInfo/getWebInfo/s:" + rootID).toString());
				json = new WsCount().getAllCount(json, rootID, webInfo.getString(rootID), "", Long.parseLong(starTime),
	...
	private String Review(String oid, int NewState) {
		...
		// 验证当前管理员是否含有审核权限
		if (userInfo != null && userInfo.size() > 0) {
			String isreview = userInfo.containsKey("isreview") ? userInfo.getString("isreview") : "0";
			...
			if (l > 0) {
				...
				return content.data(object).updateAll() > 0 ? rMsg.netMSG(true, "") : result;
			} else {
				return result;
			}
		}
	...
	private String getSingleArticle(JSONObject obj, String id) {
		...
		if (obj != null && obj.size() != 0) {
			...
			// 获取上一篇
			preobj = content.asc("_id").eq("wbid", wbid).eq("ogid", ogid).eq("slevel", 0).eq("isdelete", 0)
					.eq("isvisble", 0).gt("_id", id).field("_id,mainName").limit(1).find();
			nextobj = content.desc("_id").eq("wbid", wbid).eq("ogid", ogid).eq("slevel", 0).eq("isdelete", 0)
					.eq("isvisble", 0).lt("_id", id).limit(1).field("_id,mainName").find();
	...
	public String checkAllArticle(String perfix) {
		...
		if (condArray != null && condArray.size() > 0) {
			...
			(new Thread(() -> {
				...
				JSONArray rArray = content.or().where(condArray).and().eq("state", 2).eq("isdelete", 0)
						.eq("isvisble", 0).scan((_array) -> {
