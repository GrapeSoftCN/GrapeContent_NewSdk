...
public class WsCount {
	...
	public JSONObject getChannleCount(String wbID, long startUT, long endUT) {
		...
		// JSONArray channelArray = JSONArray.toJSONArray( (String)
		// appsProxy.proxyCall("/GrapeContent/ContentGroup/getPrevColumn/s:" + wbID,
		// null, null) );
	...
	@SuppressWarnings("unchecked")
	public JSONObject getAllCount(JSONObject robj, String rootID, String rootName, String fatherID, long startUT,
			long endUT) {
		// appsProxy.
		String[] trees = null;
		...
		long click = getClick_ckd(rootID, startUT, endUT); // 文章阅读量
		...
		// webInfoData = JSONObject.toJSON((String)
		// appsProxy.proxyCall("/GrapeWebInfo/WebInfo/getWebInfo/s:" + tree, null,
		// null));
		JSONObject newJSON = new JSONObject();
		if (trees != null) {
			JSONObject webInfos = JSONObject
					.toJSON((String) appsProxy.proxyCall("/GrapeWebInfo/WebInfo/getWebInfo/s:" + tree));
			int l = trees.length;
			for (int i = 0; i < l; i++) {
	...
	@SuppressWarnings("unchecked")
	public void sortJson_ckd(JSONArray jsonArray, String key) {
	...
	public static int partition(JSONArray array, int lo, int hi, String keyName) {
		...
		while (lo < hi) {
			...
			// ((JSONObject)array.get(lo)).put(keyName,
			// ((JSONObject)array.get(hi)).getLong(keyName));
			while (((JSONObject) array.get(lo)).getLong(keyName) >= key && hi > lo) {// 从前半部分向后扫描
				lo++;
			}
			// ((JSONObject)array.get(hi)).put(keyName,
			// ((JSONObject)array.get(lo)).getLong(keyName));
	...
	/*
	 * @SuppressWarnings("unchecked") private void sortJson(JSONArray array,int low,
	 * int hight, String key){ JSONObject json; int i, j; long index; if (low >
	 * hight) { return; } i = low; j = hight; index =
	 * ((JSONObject)array.get(i)).getLong(key); // 用子表的第一个记录做基准 while (i < j) { //
	 * 从表的两端交替向中间扫描 while (i < j && ((JSONObject)array.get(j)).getLong(key) >=
	 * index) j--;
	 * 
	 * if (i < j){// 用比基准小的记录替换低位记录 json = (JSONObject)array.get(i++); json.put(key,
	 * ((JSONObject)array.get(j)).getLong(key) ); array.set(i++, json); } while (i <
	 * j && ((JSONObject)array.get(i)).getLong(key) < index) i++;
	 * 
	 * if (i < j){ // 用比基准大的记录替换高位记录 json = (JSONObject)array.get(j--);
	 * json.put(key, ((JSONObject)array.get(i)).getLong(key) ); array.set(j--,
	 * json); } }
	 * 
	 * json = (JSONObject)array.get(i); json.put(key, index ); array.set(i, json);
	 * 
	 * sortJson(array, low, i - 1,key); // 对低子表进行递归排序 sortJson(array, i + 1, hight,
	 * key); // 对高子表进行递归排序 }
	 */
	...
	private long getCount(String wid, long startUT, long endUT) {
		...
		if (wid != null) {
			...
			count = content.eq("wbid", wid).eq("isdelete", 0).eq("isvisble", 0).count();
		}
		return count;
	}
	...
	private long getAgreeCount(String wid, long startUT, long endUT) {
		...
		if (wid != null) {
			...
			count = content.and().eq("wbid", wid).eq("state", 2).eq("isdelete", 0).eq("isvisble", 0).count();
		}
		return count;
	}
	...
	private long getDisagreeCount(String wid, long startUT, long endUT) {
		...
		if (wid != null) {
			...
			count = content.and().eq("wbid", wid).eq("state", 1).eq("isdelete", 0).eq("isvisble", 0).count();
		}
		return count;
	}
	...
	private long getClick_ckd(String wid, long startUT, long endUT) {
		...
		if (wid != null) {
			db db = content.bind().eq("isdelete", 0).eq("isvisble", 0);
			if (startUT > 0) {
